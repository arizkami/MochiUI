...
// AUKImage — image loading and drawing.
...
//
//   auto banner = AUKImage::fromFile("assets/banner.png");
//   auto icon   = AUKImage::fromResource("res://logo.png");
//
...
public:
    ...
    // ── Factories ─────────────────────────────────────────────────────────────
    static AUKImage fromFile(const std::string& path);
    static AUKImage fromMemory(const void* data, size_t size);
    static AUKImage fromResource(const std::string& resUri);  // "res://name"
...
private:
    ...
    explicit AUKImage(sk_sp<SkImage> img) : _image(std::move(img)) {}
