...
public:
    ...
    static AUKColor parse(const std::string& s) { return parseColorStr(s.c_str(), s.size()); }
    static AUKColor parse(const char* s)         { return parseColorStr(s, s ? strlen(s) : 0); }
...
private:
    ...
    static AUKColor parseColorStr(const char* s, size_t len);
    static AUKColor parseNamedColor(const char* s, size_t len);
};
...
inline AUKColor AUKColor::parseNamedColor(const char* s, size_t len) {
    ...
    while (lo < hi) {
        ...
        else return AUKColor(static_cast<SkColor>(kNames[mid].argb));
    }
    return AUKColor(); // unknown → transparent
}

inline AUKColor AUKColor::parseColorStr(const char* s, size_t len) {
    if (!s || !len) return AUKColor();

    // trim
    while (len && (*s == ' ' || *s == '\t')) { ++s; --len; }
    while (len && (s[len-1] == ' ' || s[len-1] == '\t')) --len;
    if (!len) return AUKColor();
    ...
    if (allDigits) {
        ...
        if (end && end != s) return AUKColor(static_cast<SkColor>(v));
    }
