...
sk_sp<SkImage> AUKImage::decodeFromData(sk_sp<SkData> data) {
...
AUKImage AUKImage::fromFile(const std::string& path) {
    auto data = readFileData(path);
    return AUKImage(decodeFromData(std::move(data)));
}

AUKImage AUKImage::fromMemory(const void* data, size_t size) {
    auto skData = SkData::MakeWithCopy(data, size);
    return AUKImage(decodeFromData(std::move(skData)));
}

AUKImage AUKImage::fromResource(const std::string& resUri) {
    // Strip "res://" prefix
    ...
    if (!rd.data || rd.size == 0) return AUKImage{};
...
void AUKImage::draw(SkCanvas* canvas, float x, float y,
                    SkSamplingOptions sampling) const {
    if (!_image) return;
    canvas->drawImage(_image, x, y, sampling);
}

void AUKImage::draw(SkCanvas* canvas, const SkRect& dst,
                    SkSamplingOptions sampling) const {
    if (!_image) return;
    canvas->drawImageRect(_image, dst, sampling);
}

void AUKImage::draw(SkCanvas* canvas, const SkRect& src, const SkRect& dst,
                    SkSamplingOptions sampling) const {
    if (!_image) return;
...
void AUKImage::draw(SkCanvas* canvas, const SkRect& dst, float opacity,
                    SkSamplingOptions sampling) const {
    if (!_image) return;
    SkPaint paint;
...
void AUKImage::drawFit(SkCanvas* canvas, const SkRect& dst,
                       SkSamplingOptions sampling) const {
    if (!_image) return;
...
void AUKImage::drawFill(SkCanvas* canvas, const SkRect& dst,
                        SkSamplingOptions sampling) const {
    if (!_image) return;
    float iw = static_cast<float>(_image->width());
    float ih = static_cast<float>(_image->height());
